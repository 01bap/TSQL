<script>
    ...
    onMount(() => {
        ...
        document.addEventListener("click", (event) => {
            if(event.target == closePanel && asin != "")        // Collabsing card when clicked outside
...
<div class="absolute left-0 top-0 h-full w-3/5 max-w-lg p-2 transition-[3s] hideCard" id="ProductDetailsParent">
    <div class="relative h-full w-full border rounded p-2 flex flex-col" id="ProductDetailsContainer">
        <div id="ProductDetailInfo" class="bg-sky-700/[.3] p-1 overflow-y-scroll md:overflow-y-visible rounded">
